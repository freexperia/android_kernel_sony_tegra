...
//#ifdef DEBUG_PINMUX_DUMP
//static int debug_tegra_pinmux_dump_enable = 0;

//void debug_tegra_pinmux_dump(void)
//{
//	int i;
//	int len;
//
//	if(!debug_tegra_pinmux_dump_enable) return;
//
//	for (i = 0; i < TEGRA_MAX_PINGROUP; i++) {
//		unsigned long tri;
//		unsigned long mux;
...
//		char tmp[128];
//
//		snprintf(buf, 64, "\t{TEGRA_PINGROUP_%s,\t", pingroups[i].name);
//		len = strlen(pingroups[i].name);
//
//		if (pingroups[i].mux_reg <= 0) {
//			strncat(buf, "TEGRA_MUX_NONE,\t", strlen("TEGRA_MUX_NONE,\t"));
//			len = strlen("NONE");
//		} else {
//			mux = (pg_readl(pingroups[i].mux_reg) >>
//			       pingroups[i].mux_bit) & 0x3;
//			BUG_ON(pingroups[i].funcs[mux] == 0);
//			if (pingroups[i].funcs[mux] ==  TEGRA_MUX_INVALID) {
//				strncat(buf, "TEGRA_MUX_INVALID,\t", strlen("TEGRA_MUX_INVALID,\t"));
//				len = 7;
//			} else if (pingroups[i].funcs[mux] & TEGRA_MUX_RSVD) {
//				snprintf(tmp, 64, "TEGRA_MUX_RSVD%1lu,\t", mux+1);
//				strncat(buf, tmp, strlen(tmp));
//				len = 5;
//			} else {
//				BUG_ON(!tegra_mux_names[pingroups[i].funcs[mux]]);
//				snprintf(tmp, 64,  "TEGRA_MUX_%s,\t",  tegra_mux_names[pingroups[i].funcs[mux]]);
//				strncat(buf, tmp, strlen(tmp));
//				len = strlen(tegra_mux_names[pingroups[i].funcs[mux]]);
//			}
//		}
...
//			unsigned long io;
//			io = (pg_readl(pingroups[i].mux_reg) >> 5) & 0x1;
//			snprintf(tmp, 64, "TEGRA_PIN_%s,\t", io_name(io));
//			strncat(buf, tmp, strlen(tmp));
//			len = strlen(io_name(io));
//		}
//#endif
//		if (pingroups[i].pupd_reg <= 0) {
//			strncat(buf, "TEGRA_PUPD_NORMAL,\t", strlen("TEGRA_PUPD_NORMAL,\t"));
//			len = strlen("NORMAL");
//		} else {
//			pupd = (pg_readl(pingroups[i].pupd_reg) >>
//				pingroups[i].pupd_bit) & 0x3;
//			snprintf(tmp, 64, "TEGRA_PUPD_%s,\t", pupd_name(pupd));
//			strncat(buf, tmp, strlen(tmp));
//			len = strlen(pupd_name(pupd));
//		}
//
//		if (pingroups[i].tri_reg <= 0) {
//			strncat( buf, "TEGRA_TRI_NORMAL", strlen("TEGRA_TRI_NORMAL\n"));
//		} else {
//			tri = (pg_readl(pingroups[i].tri_reg) >>
//			       pingroups[i].tri_bit) & 0x1;
//
//			snprintf(tmp, 64, "TEGRA_TRI_%s", tri_name(tri));
//			strncat(buf, tmp, strlen(tmp));
//		}
//		strncat(buf, "},\n", strlen("},\n"));
...
//EXPORT_SYMBOL(debug_tegra_pinmux_dump);

//static ssize_t debug_tegra_pinmux_dump_enable_store(struct kobject* kobj, struct kobj_attribute* attr,
//						    const char* buffer, size_t size)
//{
//	int enable;
//
//	enable = (0 < simple_strtoul(buffer, NULL, 0)) ? 1 : 0;
//	pr_info("tegra_pinmux_dump_en %d\n", enable);
//	debug_tegra_pinmux_dump_enable = enable;
//
//	return size;
//}
//static ssize_t debug_tegra_pinmux_dump_enable_show(struct kobject* kobj, struct kobj_attribute* attr,
//						   char* buffer)
//{
//	return snprintf(buffer, PAGE_SIZE, "%d", debug_tegra_pinmux_dump_enable);
//}
//static struct kobj_attribute debug_tegra_pinmux_dump_enable_attribute =
//	__ATTR(tegra_pinmux_dump_en, 0666,
//	       debug_tegra_pinmux_dump_enable_show,
//	       debug_tegra_pinmux_dump_enable_store);

//static int __init debug_tegra_pinmux_dump_init(void)
//{
//	return sysfs_create_file(kernel_kobj, &(debug_tegra_pinmux_dump_enable_attribute.attr));
//}
//late_initcall(debug_tegra_pinmux_dump_init);
//#endif /* DEBUG_PINMUX_DUMP */
