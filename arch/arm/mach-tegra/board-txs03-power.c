...
/*
 * arch/arm/mach-tegra/board-txs03-power.c
 *
...
//TPS_PDATA_INIT(vdd1, skubit0_0, 600,  1500, 0, 1, 1, 0, -1, 0, 0, EXT_CTRL_SLEPE_OFF, true);
//TPS_PDATA_INIT(vdd1, skubit0_1, 600,  1500, 0, 1, 1, 0, -1, 0, 0, EXT_CTRL_SLEEP_OFF, true);
TPS_PDATA_INIT(vdd2, 0,         1200, 1200, 0, 1, 1, 0, -1, 0, 0, 0, 0, false);
...
TPS_PDATA_INIT(ldo1, 0,         2900, 2900, tps6591x_rails(VDD_2), 1, 1, 0, 2900, 0, 1, 0, 0, false);
TPS_PDATA_INIT(ldo2, 0,         2900, 2900, tps6591x_rails(VDD_2), 0, 0, 1, -1, 0, 1, 0, 0, true);

TPS_PDATA_INIT(ldo3, e118x,     1800, 2900, 0, 0, 0, 0, -1, 0, 1, 0, 0, true);
//TPS_PDATA_INIT(ldo3, e1198,     1000, 3300, 0, 0, 0, 0, -1, 0, 0, 0, 0);
TPS_PDATA_INIT(ldo4, 0,         1000, 3300, 0, 1, 1, 0, -1, 0, 0, 0, 0, false);
TPS_PDATA_INIT(ldo5, e118x,     1800, 1800, 0, 1, 1, 1, -1, 0, 0, 0, 0, true);
//TPS_PDATA_INIT(ldo5, e1198,     2800, 2800, 0, 0, 0, 0, -1, 0, 0, 0, 0);
...
TPS_PDATA_INIT(ldo8, 0,         1000, 3300, tps6591x_rails(VIO), 1, 0, 0, -1, 0, 0, 0, 0, false);
...
static struct tps6591x_gpio_init_data tps_gpio_pdata_e1291_a04[] =  {
	...
	TPS_GPIO_INIT_PDATA(8, 1, 0, 0, 1, 0),
};
...
static struct i2c_board_info __initdata txs03_regulators[] = {
	{
...
int __init txs03_regulator_init(void)
{
	...
	i2c_register_board_info(4, txs03_regulators, 1);
...
/* EN_5V0_BOOST */
static struct regulator_consumer_supply gpio_switch_en_5v0_boost_supply[] = {
	REGULATOR_SUPPLY("en_5v0_boost", NULL),
};
static int gpio_switch_en_5v0_boost_voltages[] = { 5000};
...
GREG_INIT(9, en_3v3_fuse,	en_3v3_fuse,	"vdd_3v3_devices",	0,      0,      TEGRA_GPIO_PI2,		false,	0,	0,	0,	0);
...
GREG_INIT(13, en_1v8_cam,	en_1v8_cam,	"vdd_gen1v8",		0,      0,      TEGRA_GPIO_PR5,	false,	0,	0,	0,	0);

/* EN_5V0_BOOST */
GREG_INIT(12, en_5v0_boost,     en_5v0_boost,   NULL,                   0,      0,       TPS6591X_GPIO_8,       false,  0,      0,      0,      0);
...
GREG_INIT(15, en_usb1_vbus_oc_e118x,	en_usb1_vbus_oc,	"en_5v0_boost",
	  0,      0,      TEGRA_GPIO_PH5,		false,	0,	0,      0,      0);
GREG_INIT(16, en_usb3_vbus_oc_e118x,	en_usb3_vbus_oc,	NULL,
		0,      0,      TEGRA_GPIO_PD1,		false,	0,	TEGRA_PINGROUP_GMI_AD15,
		enable_load_switch_rail, disable_load_switch_rail);
...
/*GREG_INIT(15, en_usb1_vbus_oc,		en_usb1_vbus_oc,	"vdd_5v0_sys",
		0,      0,      TEGRA_GPIO_PH5,		false,	0,	TEGRA_PINGROUP_GMI_RST_N,
		enable_load_switch_rail, disable_load_switch_rail);
...
int __init txs03_gpio_switch_regulator_init(void)
{
	int i;
...
static void txs03_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void txs03_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data txs03_suspend_data = {
	.cpu_timer	= 2000,
	.cpu_off_timer	= 200,
	...
	.board_suspend = txs03_board_suspend,
	.board_resume = txs03_board_resume,
};

int __init txs03_suspend_init(void)
{
	...
	tegra_init_suspend(&txs03_suspend_data);
	return 0;
}

static void txs03_power_off(void)
{
	int ret;
	pr_err("txs03: Powering off the device\n");
	ret = tps6591x_power_off();
	if (ret)
		pr_err("txs03: failed to power off\n");

	while (1);
}

static void txs03_pm298_power_off(void)
{
	int ret;
	pr_err("txs03-pm298: Powering off the device\n");
	ret = max77663_power_off();
	if (ret)
		pr_err("txs03-pm298: failed to power off\n");

	while (1);
}

int __init txs03_power_off_init(void)
{
	struct board_info pmu_board_info;

	//	tegra_get_pmu_board_info(&pmu_board_info);

	pm_power_off = txs03_power_off;
...
void __init txs03_tsensor_init(void)
{
...
int __init txs03_edp_init(void)
{
