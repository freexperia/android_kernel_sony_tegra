...
// EN_3V3_COM  GPIO_PP1
#define EN_3V3_COM   TEGRA_GPIO_PP1

// WLAN_CHIP_PWD_N  GPIO_PP0
#define GPIO_WLAN_CHIP_PWD_N   TEGRA_GPIO_PP0

// BT_RST_N         GPIO_PU0
#define GPIO_BT_RST_N          TEGRA_GPIO_PU0

// GPS_PWR_EN       GPIO_PU2
#define GPIO_GPS_PWR_EN        TEGRA_GPIO_PU2

// WWAN_PWR_EN      GPIO_PD2
#define GPIO_WWAN_PWR_EN       TEGRA_GPIO_PD2

// WWAN_RF_ON_N     GPIO_P S1      H/z=off L=on
#define GPIO_WWAN_RF_ON_N      TEGRA_GPIO_PS1
...
static int txs03_gpio_request(int gpio_num, const char *gpio_name, int direction, int value)
{
	int ret;
...
static int init(void)
{
	...
	{
		tegra_gpio_enable(TEGRA_GPIO_PP1);	
		ret = gpio_request(TEGRA_GPIO_PP1, "TEGRA_GPIO_PP1");
		if (ret < 0) {
			printk("gpio req err: TEGRA_GPIO_PP1\n");
			return ret;
		}
		ret = gpio_direction_output(TEGRA_GPIO_PP1, 1);
		if (ret < 0) {
			printk("gpio dir err: TEGRA_GPIO_PP1\n");
			gpio_free(TEGRA_GPIO_PP1);
			return ret;
		}
		gpio_set_value(TEGRA_GPIO_PP1, 1);
	}

	{
		...
		if (s_hWlanChipPwdN_Pin == false) {
			ret = txs03_gpio_request(port, "WlanChipPwdN", GPIODIRECTION_OUT, value);
			if (ret < 0) {
				PRINT_WIRELESSPOWER(("fail:s_hWlanChipPwdN_Pin = txs03_gpio_request( s_hGpio, port, pin );\n"));
				return ret;
			}
		}
	...
	
	{
		...
		if (s_hBtRstN_Pin == false) {
			ret = txs03_gpio_request(port, "BtRstN", GPIODIRECTION_OUT, value);
			if (ret < 0) {
				PRINT_WIRELESSPOWER(("fail:s_hBtRstN_Pin = txs03_gpio_request( s_hGpio, port, pin );\n"));
				return ret;
			}
		}
		gpio_set_value(port, value);
	...
	
	{
		...
		if (s_hGpsPwrEn_Pin == false) {
			ret = txs03_gpio_request(port, "GpsPwrEn", GPIODIRECTION_OUT, value);
			if (ret < 0) {
				PRINT_WIRELESSPOWER(("fail:s_hGpsPwrEn_Pin = txs03_gpio_request( s_hGpio, port, pin );\n"));
				return ret;
			}
		}
		gpio_set_value(port, value);
	...
	
	{
		...
		if (s_hWWanRfOnN_Pin == false) {
			ret = txs03_gpio_request(port, "WWanRfOnN", GPIODIRECTION_OUT, value);
			if (ret < 0) {
				PRINT_WIRELESSPOWER(("fail:s_hWWanRfOnN_Pin = txs03_gpio_request( s_hGpio, port, pin );\n"));
				return ret;
			}
		}
		gpio_set_value(port, value);
	...
	
	{
		...
		if (s_hWWanPwrEn_Pin == false) {
			ret = txs03_gpio_request(port, "WWanPwrEn", GPIODIRECTION_OUT, value);
			if (ret < 0) {
				PRINT_WIRELESSPOWER(("fail:s_hWWanPwrEn_Pin = txs03_gpio_request( s_hGpio, port, pin );\n"));
				return ret;
			}
		}
		gpio_set_value(port, value);
