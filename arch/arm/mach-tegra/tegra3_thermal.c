...
#ifdef TXS03_THROTTLE_TEMPERATURE
...
#ifdef TXS03_THROTTLE_TEMPERATURE
#define THROTTLE_LIMIT_OFFSET 3000
static long temp_offset;
#endif
...
static int tegra_thermal_zone_get_trip_temp(struct thermal_zone_device *thz,
	...
	*temp = tj2dev(thermal->device, thermal->temp_throttle_tj);
	...
	if (thermal->device->throttle_ex) {
		err = thermal->device->throttle_ex->get_local_temp(thermal->device->data, &local_temp);
		if (err) {
			pr_err("%s: get local temp fail(%d)", __func__, err);
		} else {
			for (i = 0; i < (thermal->device->throttle_ex->threshold_table_size); i++) {
				if (local_temp > thermal->device->throttle_ex->threshold_table_tj[i].local_threshold) {
					*temp = thermal->device->throttle_ex->threshold_table_tj[i].remote_threshold;
					break;
				}
			}
		}
...
static int tegra_thermal_throttle_temp_tj_set(void *data, u64 val)
{
	...
	if( val > (thermal_state.temp_shutdown_tj - THROTTLE_LIMIT_OFFSET) ){
		val = thermal_state.temp_shutdown_tj - THROTTLE_LIMIT_OFFSET;
	}
#endif /* TXS03_THROTTLE_TEMPERATURE */
...
int tegra_get_temp_throttle( long *val )
{
	int ret;
	u64 temp = 0;
	ret = tegra_thermal_throttle_temp_tj_get(NULL, &temp);
	*val = temp - temp_offset;
	return ret;
}

int tegra_set_temp_throttle( long val )
{
	return tegra_thermal_throttle_temp_tj_set(NULL, (u64)(val + temp_offset));
}
