...
#if defined(CONFIG_MACH_NBX02)
unsigned long tegra_bootloader_fb_start = 0x1c013000;
unsigned long tegra_bootloader_fb_size = SZ_4M;
#else
...
static void tegra_pm_restart(char mode, const char *cmd)
{
	...
	local_irq_disable();
	while(1) tegra_gpio_init_configure(TEGRA_GPIO_PS0, false, 1);
#endif
