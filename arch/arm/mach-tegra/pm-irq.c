...
/* any polarity wake for USB1 and USB2 VBUS and ID wake sources */
#define USB_ANY_POLARITY_MASK	(0xFULL << 19)
...
{
	u64 wake_msk;
	int err = tegra_irq_to_wake(irq, &wake_msk);
	...
	if (enable) {
		tegra_lp0_wake_enb |= wake_msk;
		/* handling USB any polarity wake sources - VBUS and ID */
		if (wake_msk & USB_ANY_POLARITY_MASK)
			tegra_lp0_wake_level_any |= (wake_msk &
				USB_ANY_POLARITY_MASK);
	} else {
		tegra_lp0_wake_enb &= ~wake_msk;
	}
