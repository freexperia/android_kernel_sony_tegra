				...
				enum tegra_usb_phy_port_speed port_speed)
{
	...
	if( phy->instance==0 ){
	/* We expect either J or K on the bus. If it is SE0 it might mean that */
	/* the device was removed. Don't enable OBS in this case*/
	if (!((readl(base + USB_PORTSC1) >> 10) & 0x3)) {
		val = readl(base + UTMIP_MISC_CFG0);
