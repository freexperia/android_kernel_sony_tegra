...
#define T_1250us       (1250000)
#define T_2500us       (2500000)
#define T_5000us       (5000000)
#define T_10000us      (10000000)
...
#define FIFO_BLOCK_XFER_MAX	(I2C_SMBUS_BLOCK_MAX / 6)

static inline s32 read_fifo_data(struct l3g4200dh_data* gyro, int count)
{
	int i;
	int n;
	s32 err;
	...
	for (i = 0; i < count; ) {
		n = count - i;
		if (n > FIFO_BLOCK_XFER_MAX) {
			n = FIFO_BLOCK_XFER_MAX;
		}
		err = i2c_smbus_read_i2c_block_data(g_client, 
				REG_OUTS, 6 * n, 
				(u8*)&gyro->buffer[i*3]);
		if (err < 0) {
...
static inline s32 read_data(struct l3g4200dh_data *gyro, int count)
{
	int i;
	s32 err;
	s16 x, y, z;
	...
	err = read_fifo_data(gyro, count);
	if (err < 0) {
		goto out;
	}
	R = gyro->plat_data.transformation_matrix;
	values = (u8*)gyro->buffer;
	for (i = 0; i < count; ++i) {
		...
		gyro->buffer[3*i]   = xx;
		gyro->buffer[3*i+1] = yy;
		gyro->buffer[3*i+2] = zz;
		values += 6;
	}
out:
	return err;
...
static void work_func(struct work_struct *work)
{
	...
	err = data = _i2c_read_byte(g_client, REG_FIFO_SRC_REG);
	if (err < 0) {
		goto out;
	}
	count = data & 0x1f;
	...
	diff = nsec - gyro->prev_nsec;
	gyro->prev_nsec = nsec;


	if (diff >= (gyro->cycle * count)) {
		t = gyro->cycle;
	} else {
		/* diff < gyro->cycle * 32 < INT_MAX */
		t = (s32)diff / count;
	}
	err = read_data(gyro, count);
	if (err < 0) {
		goto out;
	}
	nsec -= t * (count - 1);
	for (i = 0; i < count; ++i) {
		...
		input_report_abs(gyro->input_dev, ABS_RX, 
					nsec & 0xffffffff);
		input_report_abs(gyro->input_dev, ABS_RY, 
					nsec >> 32);
...
static void clear_fifo(struct l3g4200dh_data *gyro)
{
       pr_debug("called\n");
       _i2c_write_byte(g_client, REG_FIFO_CTRL_REG, 0x00);
       _i2c_write_byte(g_client, REG_FIFO_CTRL_REG, 0x41);
}

#else /* not CONFIG_NBX_L3G4200DH_HIGHSPEED */
...
#endif /* not CONFIG_NBX_L3G4200DH_HIGHSPEED */
...
#ifdef CONFIG_NBX_L3G4200DH_HIGHSPEED
	queue_work(gyro->workqueue, &gyro->work);
#else
...
#ifdef CONFIG_NBX_L3G4200DH_HIGHSPEED
	clear_fifo(gyro);
	_i2c_write_byte(g_client, REG_CTRL_REG5, 0x40);
	_i2c_write_byte(g_client, REG_CTRL_REG1, gyro->ctrl_reg1_drbw | 0x0f);
#else
...
#ifdef CONFIG_NBX_L3G4200DH_HIGHSPEED
	flush_workqueue(gyro->workqueue);
#endif
					...
					const struct i2c_device_id *id)
{
	...
	case 200:
		gyro->cycle = T_5000us;
		gyro->ctrl_reg1_drbw = 0x40;
		break;
	case 400:
		gyro->cycle = T_2500us;
		gyro->ctrl_reg1_drbw = 0x80;
		break;
	case 800:
		gyro->cycle = T_1250us;
		gyro->ctrl_reg1_drbw = 0xc0;
		break;
	case 100:
	default:
		gyro->cycle = T_10000us;
		gyro->ctrl_reg1_drbw = 0x00;
		break;
	}

	gyro->workqueue = alloc_workqueue("l3g4200dh",
			WQ_HIGHPRI | WQ_UNBOUND | WQ_RESCUER, 1);
	if (!gyro->workqueue) {
		pr_err("alloc_workqueue()\n");
		retval = -ENOMEM;
	...
	input_set_abs_params(gyro->input_dev, ABS_X, SHRT_MIN, SHRT_MAX, 0, 0);
	input_set_abs_params(gyro->input_dev, ABS_Y, SHRT_MIN, SHRT_MAX, 0, 0);
	input_set_abs_params(gyro->input_dev, ABS_Z, SHRT_MIN, SHRT_MAX, 0, 0);
#ifdef CONFIG_NBX_L3G4200DH_HIGHSPEED
	input_set_abs_params(gyro->input_dev, ABS_RX, INT_MIN, INT_MAX, 0, 0);
	input_set_abs_params(gyro->input_dev, ABS_RY, INT_MIN, INT_MAX, 0, 0);
#endif
...
#ifdef CONFIG_NBX_L3G4200DH_HIGHSPEED
	destroy_workqueue(gyro->workqueue);
err_alloc_workqueue:
#endif
	kfree(gyro);
err_kzalloc:
