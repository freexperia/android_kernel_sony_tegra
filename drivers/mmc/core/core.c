...
struct mmc_host *static_sd_host = NULL;
extern struct mmc_host *static_host;
...
void mmc_fake_rescan(struct mmc_host *host){
	...
	err = mmc_send_io_op_cond(static_host, 0, &ocr);
	if (!err) {
		mmc_attach_sdio(static_host);
...
void set_static_loaded(int value){
	mutex_lock(&static_loaded_lock);
	static_loaded = value;
	mutex_unlock(&static_loaded_lock);
}
