...
static bool sdhci_busy_wakelock_init = false;
static struct wake_lock sdhci_busy_wakelock;
				  ...
				  struct sdhci_pltfm_data *pdata)
{
	...
	/* */
	if ( !sdhci_busy_wakelock_init ) {
		wake_lock_init(&sdhci_busy_wakelock, WAKE_LOCK_SUSPEND, "sdhci_busy_tegra");
		sdhci_busy_wakelock_init = true;
	}
...
static int tegra_sdhci_suspend(struct sdhci_host *sdhci, pm_message_t state)
{
	...
	{
		int nr_writeback = global_page_state(NR_WRITEBACK);
		if ( nr_writeback ) {
			dev_err(mmc_dev(sdhci->mmc), "%d writeback pages found, suspend aborted\n", nr_writeback);
			wake_lock_timeout(&sdhci_busy_wakelock, 3 * HZ);
			return -EBUSY;
		}
	}

	if ( sdhci->mmc->claim_cnt ) {
		dev_err(mmc_dev(sdhci->mmc), "mmc_host claimed, suspend aborted\n");
		return -EBUSY;
	}
