...
static struct wake_lock hdmi_detect_wake_lock;
#endif
...
static bool tegra_dc_hdmi_detect(struct tegra_dc *dc)
{
	...
	int err;
#ifdef WAKELOCK_HDMI
	/* If tegra_dc_hdmi_hpd() is false, it is unnecessary to wakelock.
	   With this flag, we check whether we use wake_lock_timeout on the end of this function. */
	int wakelock_flag = 0;
#endif
#ifdef RETRY_READ_EDID
	int retries = 3;
#endif
...
#ifdef WAKELOCK_HDMI
	wake_lock(&hdmi_detect_wake_lock);
	wakelock_flag = 1;
#endif
#ifdef RETRY_READ_EDID
	do {
		...
		if ((err < 0) && (retries > 1)){
			dev_err(&dc->ndev->dev, "retry reading edid\n");
			msleep(250);
		}
	}while ((err < 0) && retries--);
#else
...
#ifdef WAKELOCK_HDMI
	wake_lock_timeout(&hdmi_detect_wake_lock, 15 * HZ);
#endif
	return true;
...
#ifdef WAKELOCK_HDMI
	wake_lock_init(&hdmi_detect_wake_lock, WAKE_LOCK_SUSPEND, "hdmi_detect");
#endif
	return 0;
...
#ifdef WAKELOCK_HDMI
	wake_lock_destroy(&hdmi_detect_wake_lock);
#endif
	free_irq(gpio_to_irq(dc->out->hotplug_gpio), dc);
