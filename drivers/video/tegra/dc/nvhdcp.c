/* 2012-09-18: File changed by Sony Corporation */
/*
...
static struct wake_lock nvhdcp_wake_lock;
#endif
...
static void nvhdcp_downstream_worker(struct work_struct *work)
{
	...
	u32 res;
#ifdef WAKELOCK_HDMI
	/* If hdmi losts or nvhdcp failure, it is unnecessary to wakelock.
...
#ifdef WAKELOCK_HDMI
	wake_lock_timeout(&nvhdcp_wake_lock, 15 * HZ);
	wakelock_flag = 0;
#endif
	while (1) {
		if (!nvhdcp_is_plugged(nvhdcp))
...
#ifdef WAKELOCK_HDMI
	wake_lock_init(&nvhdcp_wake_lock, WAKE_LOCK_SUSPEND, "nvhdcp");
#endif
	return nvhdcp;
free_workqueue:
...
{
#ifdef WAKELOCK_HDMI
	wake_lock_destroy(&nvhdcp_wake_lock);
#endif
