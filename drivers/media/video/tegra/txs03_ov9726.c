...
ov9726_power(struct ov9726_devinfo *dev, bool pwr_on)
{
	...
	/*
	 * execute the appropriate board-specific power management function.
	 */
	nRetCode = pwr_on ? dev->pdata->power_on() : dev->pdata->power_off();

	/*
	 * power management function returned an error?
	 */
	if (nRetCode)
		dev_err(&i2c_client->dev,
		        "ov9726: power %s failed (error %i)\n",
		        pwr_on ? "on" : "off",
		        nRetCode);
...
static int ov9726_set_group_hold(struct i2c_client *info, struct tegra_camera_ae *ae)
{
	int ret;
	int count = 0;
	...
	if (groupHoldEnabled) {
		ret = ov9726_write_reg8(info, 0x104, 0x01);
		if (ret)
			return ret;
	}

	if (ae->gain_enable)
		ov9726_set_gain(info, ae->gain);
	if (ae->coarse_time_enable)
		ov9726_set_coarse_time(info, ae->coarse_time);
	if (ae->frame_length_enable)
		ov9726_set_frame_length(info, ae->frame_length);

	if (groupHoldEnabled) {
		ret = ov9726_write_reg8(info, 0x104, 0x00);
		if (ret)
			return ret;
	}
...
ov9726_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case OV9726_IOCTL_SET_GROUP_HOLD:
	{
		...
		return ov9726_set_group_hold(i2c_client, &ae);
	}
...
{
	pr_debug(KERN_DEBUG "ov9726 sensor driver loading\n");
