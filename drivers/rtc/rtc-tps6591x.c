...
/*
 txs03
 lower year limit for set date
*/
#define TXS03_LOWER_YEAR  2000
#define TXS03_LOWER_MONTH 1
#define TXS03_LOWER_DAY   1
#define TXS03_LOWER_HOUR  0
#define TXS03_LOWER_MIN   0
#define TXS03_LOWER_SEC   0
...
static void print_time_dbg(struct device *dev, struct rtc_time *tm)
{
	dev_dbg(dev, "RTC Time : %d/%d/%d %d:%d:%d\n",
		(tm->tm_mon + 1), tm->tm_mday, (tm->tm_year + OS_REF_YEAR),
		tm->tm_hour, tm->tm_min , tm->tm_sec);
}
...
static int tps6591x_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (seconds < rtc->lower_limit) {
		dev_info(dev, "requested time unsupported\n");
		return -EINVAL;
	}
#endif
...
#if defined(CONFIG_MACH_TXS03)
	rtc->lower_limit = mktime(TXS03_LOWER_YEAR, TXS03_LOWER_MONTH, TXS03_LOWER_DAY,
				TXS03_LOWER_HOUR, TXS03_LOWER_MIN, TXS03_LOWER_SEC);
#endif
...
#if defined(CONFIG_MACH_TXS03)
	err = tps6591x_rtc_read_time(&pdev->dev, &tm);
	if (err || (tm.tm_year < RTC_YEAR_OFFSET || tm.tm_year > alarm_set_rtc_upper_limit.tm_year)){
		memset(&tm, 0, sizeof(tm));
		tm.tm_year = TXS03_LOWER_YEAR - OS_REF_YEAR;
		tm.tm_mon = TXS03_LOWER_MONTH - 1;
		tm.tm_mday = TXS03_LOWER_DAY;
		tm.tm_hour = TXS03_LOWER_HOUR;
		tm.tm_min = TXS03_LOWER_MIN;
		tm.tm_sec = TXS03_LOWER_SEC;
	}
	err = tps6591x_rtc_set_time(&pdev->dev, &tm);
	if (err) {
