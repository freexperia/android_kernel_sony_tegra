...
static void invalidate_cpufreqpolicy(struct cpufreq_policy* p)
{
	memset(p, 0x6b, sizeof(*p));
	p->governor = 0;
	p->user_policy.governor = 0;
	p->kobj.name = 0;
	p->kobj.parent = 0;
	p->kobj.kset = 0;
	p->kobj.ktype = 0;
			    ...
			    unsigned int relation)
{
	int retval = -EINVAL;
	unsigned int cpu;
	if ( !policy || policy->cpu >= nr_cpumask_bits ) {
		return retval;
	}
	cpu = policy->cpu;
	dprintk("target for CPU %u: %u kHz, relation %u\n", cpu,
		target_freq, relation);
	if (cpu_online(cpu) && cpufreq_driver->target)
