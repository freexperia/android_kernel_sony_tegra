	...
	int *rbuf)
{
	...
	dev_vdbg(&akm->input->dev, "  Rotation Vector  : %6d,%6d,%6d,%6d",
	...
	/* Report rotation vector information */
	if (ready & ROT_DATA_READY) {
		input_report_abs(akm->input, ABS_HAT2X, rbuf[12]);
		input_report_abs(akm->input, ABS_HAT2Y, rbuf[13]);
		input_report_abs(akm->input, ABS_HAT3X, rbuf[14]);
		input_report_abs(akm->input, ABS_HAT3Y, rbuf[15]);
	}
