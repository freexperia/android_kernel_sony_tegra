...
static int nct1008_get_local_temp(struct device *dev, long *pTemp)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct nct1008_platform_data *pdata = client->dev.platform_data;
	s8 temp_local;
	...
	if (value < 0)
		goto error;
	temp_local = value_to_temperature(pdata->ext_range, value);
	*pTemp = CELSIUS_TO_MILLICELSIUS(temp_local);
...
int nct1008_thermal_get_local_temp(struct nct1008_data *data, long *temp)
{
	return nct1008_get_local_temp(&data->client->dev, temp);
}
