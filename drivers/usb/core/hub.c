		...
		int retry_counter)
{
	...
	/* [WAR] Force retry on miss speed detection of internal device */
	if (!udev->config) {
		extern bool tegra_ehci_is_internal_bus(struct usb_bus*);
		if (tegra_ehci_is_internal_bus(udev->bus)
			&& udev->speed != USB_SPEED_HIGH) {
			dev_info(&udev->dev, "[WAR] Retry expecting high speed detection.\n");
			goto fail;
		}
	}
