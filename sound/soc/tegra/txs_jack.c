...
#define  _COUNT_DEFAULT		(100)    /* ms */
#define  _COUNT_TRANSITION	(200)   /* ms */
#define  _COUNT_SP          (600)   /* ms */

#define  _MIC_BIAS_ON	1
#define  _MIC_BIAS_OFF	0
...
static void
_set_mic_bias_enable(unsigned int flag)
{
	if( _MIC_BIAS_ON == flag ) {
		snd_soc_update_bits(nbx_jack->codec, WM8903_MIC_BIAS_CONTROL_0,
			WM8903_MICBIAS_ENA, WM8903_MICBIAS_ENA);
	} else {
		snd_soc_update_bits(nbx_jack->codec, WM8903_MIC_BIAS_CONTROL_0,
			WM8903_MICBIAS_ENA, 0);
	}
}
...
_state_machine(void)
{
	...
	if ( next_state != nbx_jack->state ) { /* state changed */
		...
		case _NBX_JACK_STATE_SP:
			...
			_set_mic_bias_enable(_MIC_BIAS_OFF);
			break;
		case _NBX_JACK_STATE_HP:
		...
		case _NBX_JACK_STATE_TRANSITION:
			...
			_start_timer();
			_set_mic_bias_enable(_MIC_BIAS_ON);
			break;
		default:
			printk(KERN_ERR "nbx_jack: unknown state: %d\n", next_state);
			break;
...
_polling_gpio(void)
{
	...
	if ( nbx_jack->ctr > 0 ) {
	  //printk(KERN_ERR "XXX: nbx_jack: _polling_gpio: ctr: %d\n", nbx_jack->ctr);
		if ( new_is_plugged && nbx_jack->state == _NBX_JACK_STATE_SP ) {
			expired_time = _COUNT_SP;
		} else if ( new_is_plugged && nbx_jack->state == _NBX_JACK_STATE_TRANSITION ) {
			expired_time = _COUNT_TRANSITION;
		} else {
			expired_time = _COUNT_DEFAULT;
		}
...
nbx_jack_resume(void)
{
	...
	if ( !nbx_jack->suspended ) return 0;
	nbx_jack->suspended = false;
