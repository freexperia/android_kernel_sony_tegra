...
#ifdef SONY
	ret = ath6kl_wmi_scanparams_cmd(ar->wmi, 0, 0, 0, 60, 60, 60, 0, 3, 0, 0, 0);
	if (ret) {
		ath6kl_warn("Failed to set scanparam : %d\n",ret);
	}
#endif
				...
				struct cfg80211_scan_request *request)
{
	...
	u16 *channels = NULL;
	int ret = 0;
	u32 force_fg_scan = 1;  // use multi-channel scan by default
...
#ifdef SONY
	ath6kl_cfg80211_stop_all_deepsleep(ar);
#else
...
#ifdef SONY
#define WLAN_REASON_DEAUTH_DUE_TO_DEEPSLEEP 1024
void ath6kl_cfg80211_stop_deepsleep(struct ath6kl_vif *vif)
{
	ath6kl_cfg80211_sscan_disable(vif);
	...
	case SME_CONNECTING:
		...
		break;
	case SME_CONNECTED:
		cfg80211_disconnected(vif->ndev, WLAN_REASON_DEAUTH_DUE_TO_DEEPSLEEP, NULL, 0, GFP_KERNEL);
		break;
	}
...
void ath6kl_cfg80211_stop_all_deepsleep(struct ath6kl *ar)
{
	struct ath6kl_vif *vif;
	 ...
	 */
	list_for_each_entry(vif, &ar->vif_list, list)
		ath6kl_cfg80211_stop_deepsleep(vif);
}
#endif
