...
#ifdef SONY
static int __ath6kl_sdio_suspend(struct ath6kl *ar, struct cfg80211_wowlan *wow)
#else
...
#endif
{
	...
	if (ar->suspend_mode == WLAN_POWER_STATE_DEEP_SLEEP ||
		...
		ret = ath6kl_cfg80211_suspend(ar, ATH6KL_CFG_SUSPEND_DEEPSLEEP,
					      NULL);
		if (ret) {
			/* disable cut power suspend since this is not supported by sony
			goto cut_pwr; */
...
{
	int ret;
	ret = __ath6kl_sdio_suspend(ar, wow);
